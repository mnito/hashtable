/*****************************************************
* Hash Table Command Line Interface                  *
...
static bool _remove(char *input, HASHTABLE ht)
{
    char *key;
    key = (char*)malloc(sizeof(char) * MAX_KEY);
    if(sscanf(input, "remove %s", key) < 1) return 0;
...
int main(int argc, char **argv)
{
    ...
    while(1) {
        ...
        if(!(process(input, &ht))) puts("Invalid input. Type help for commands.");
    }
    return 0;
}
